...
def train_isolation_forest(X: np.ndarray, name: str, contamination=0.05):
    ...
    # FIX #6: use the same formula as online_scoring.py: clip(1 + raw, 0, 1)
    # Previously used min-max normalisation which produced different scale at inference.
    raw_scores  = model.score_samples(X_scaled)
    scores_norm = np.clip(1.0 + raw_scores, 0.0, 1.0)  # higher = more anomalous
    ...
    # Threshold: 95th / 99th percentile of training scores
...
def save_models(motion_model, motion_scaler, motion_tw, motion_tc,
                ...
                df, vitals_model_type="autoencoder"):  # FIX #7: accept type here
    ...
    meta = {
        ...
        "vitals_model_type": vitals_model_type,  # FIX #7: included in first write
        "thresholds": {
...
def verify_models(meta):
    ...
    normal_motion = np.array([[2, 0.3, 120, 0.5, 0.866]])
    ...
    # FIX #12: normalise autoencoder raw MSE against vitals_crit threshold
    crit = meta["thresholds"]["vitals_crit"]
    s_vitals_raw = score_autoencoder(ae, vitals_scaler, normal_vitals)[0]
    s_vitals = float(np.clip(s_vitals_raw / (crit + 1e-9), 0.0, 1.0))
...
def main():
    ...
    # ── Save ──  FIX #7: vitals_model_type is passed in and saved atomically
    meta = save_models(
        motion_model, motion_scaler, motion_tw, motion_tc,
        adl_model,    adl_scaler,    adl_tw,    adl_tc,
        vitals_model, vitals_scaler, vitals_tw, vitals_tc,
        df, vitals_model_type=vitals_model_type,
    )
